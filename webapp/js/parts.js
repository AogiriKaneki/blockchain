...
window.onload = async function () {
    ...
    document.getElementById("build-part").addEventListener("click", function () {
        ...
        var part_sha = web3.utils.soliditySha3(
            {type:'address', value : window.accounts[0]},
            {type:'uint', value : web3.utils.fromAscii(serial)},
            {type:'bytes32',value : web3.utils.fromAscii(part_type)},
            {type:'bytes',value : web3.utils.fromAscii(creation_date)}
            );
        window.inst_pm.methods.buildPart(serial, part_type, creation_date).send(
