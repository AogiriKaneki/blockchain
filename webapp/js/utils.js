...
function populateDetails(item) {
    ...
    window.inst_pm.methods.parts(item).call({ from: window.accounts[0] }, function (error, part_info) {
        if (error)
            console.log(error)
        else {
            ...
            window.co.methods.CurrentPartOwner(item).call({ from: window.accounts[0] }, function (error, result) {
                if (error) {
                    ...
                    if (result == "0x0000000000000000000000000000000000000000") {
                        ...
                        window.co.methods.CreateOwnership(0, item).send({ from: window.accounts[0], gas: 300000 }, function (error, result) {
                            if (error) {
                                console.log(error)
                            } else {
...
function populateCarDetails(item) {
    ...
    console.log(item)
    //Query blockchain for data to fill element
    window.inst_pm.methods.products(item).call({ from: window.accounts[0] }, function (error, prod_info) {
        if (error)
            console.log(error)
        else {
                    ...
                    window.co.methods.CurrentProductOwner(item).call({ from: window.accounts[0] }, function (error, result) {
                        if (error) {
                            ...
                            if (result == "0x0000000000000000000000000000000000000000") {
                                ...
                                window.co.methods.CreateOwnership(1, item).send({ from: window.accounts[0], gas: 300000 }, function (error, result) {
                                    if (error) {
                                        console.log(error)
                                    } else {
...
async function init_web3() {
    ...
    if (typeof web3 != 'undefined') {
        ...
        console.log("web3 being used"); // what Metamask injected 
    } else {
        // Instantiate and set Ganache as your provider
        web3 = new Web3(new Web3.providers.HttpProvider("http://localhost:8545"));
        console.log("ganache being used"); // what Metamask injected 
    }
    //Load accounts
    ...
    window.inst_pm = new web3.eth.Contract([
        {
            "constant": true,
            "inputs": [
                {
    ...
    window.inst_pm.options.address = '0x0deedfa13d33fc739b7b009e63138e6dba663179'
...
function eWSPartListManager() {
    toggleActive(this)
    clearActiveExcept(this)

    if (this.classList.contains("active")) {
        getOwnerHistoryFromEvents('TransferOwnership_part', this.textContent).then((owners) => {
            console.log("bbb")
            console.log(owners)
...
function dealerProductListManager() {
    ...
    if (this.classList.contains("active")) {
        getOwnerHistoryFromEvents('TransferOwnership_product', this.textContent).then((owners) => {
